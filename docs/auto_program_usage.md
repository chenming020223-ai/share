...
当前默认使用深度模式：除比赛、赔率、双方近 10 场赛果、赛季统计和交锋外，还会对双方近期比赛补抓技术统计和事件数据，用于形成接近历史工作簿的数据口径。
...
```bash
python3 -m worldcup_predictor --auto --home Brazil --away France --collection-mode deep
```

批量建库模式用于后台任务：

```bash
python3 -m worldcup_predictor --batch-collect-today --scope first_division --collection-mode batch
```
...
默认均注金额使用自动资金策略：启动资金 1000 元分 5 份，即首阶段每个正式方向 200 元；盈利后只把利润的 50% 纳入重新分配，亏损时按当前资金重新五等分。
...
含义：模型概率至少比市场概率高 8 个百分点。正式 API 模式使用庄家优先级全场盘口；双方须各有至少 5 场有效近期比赛；研究试算 EV 不低于 5%、保守研究试算 EV 不低于 3%。若胜平负中任一方向的基础模型概率与当前市场基准去水概率差异超过 15 个百分点，整场三类市场 EV 暂停展示。胜平负方向还要求基础模型概率至少为 40%。
...
注意：`--force-picks` 只保留给命令行调试。正式网页交付版会保留通过庄家优先级全场盘口、近期数据、概率分歧、优势门槛和保守研究试算 EV 校验的研究方向；`pfinal` 校准验收前，API 模式不会输出正式模拟信号或占用资金。
...
|---|---|---:|
| 快速模式 | 临时省额度预测 | 通常十几次 |
| 深度模式 | 单场默认预测与报告 | 通常几十次 |
| 批量建库模式 | 每天后台积累样本 | 取决于候选比赛数量，可达到几百到几千次 |
