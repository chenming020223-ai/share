...
| `qmkt` | 指定庄家赔率去水后的市场概率 | 当前 `market_probabilities` | 否 |
...
研究试算 EV 仅用于诊断基础模型与市场的分歧，在 `pfinal` 可用前不得触发正式模拟信号。若 Pinnacle 胜平负市场中任一方向的 `pbase` 与 `qmkt` 绝对差异超过 15 个百分点，整场比分分布视为存在重大分歧：胜平负、大小球和让球的研究 EV 主展示全部暂停，仅在报告审计附录保存原始试算值。
...
| `bookmaker` | 正式 API 模式固定为 `Pinnacle` |
...
现有盘口、数据质量和保守 EV 校验继续保留，但增加概率治理硬门槛：
...
4. 保留每笔信号的 `snapshot_id`、指定庄家赔率、真实抓取时间和结算规则。
